 ...
 * @class
 */
export default function RtlCom(conn) {
  /**
   ...
   * @returns {ArrayBuffer} The read buffer.
   */
   ...
   * @param {ArrayBuffer} buffer The buffer to write.
   */
  async function _writeI2CRegBuffer(addr, reg, buffer) {
  ...
  async function readCtrlMsg(value, index, length) {
    ...
    try {
      ...
      if (VERBOSE) {
        console.log('IN value 0x' + value.toString(16) + ' index 0x' + index.toString(16));
   ...
   * @param {number} length The number of bytes to read.
   * @returns {ArrayBuffer} The received buffer.
   */
  async function readBulk(length) {
    ...
    try {
      ...
      var msg =
        'USB bulk read failed (length 0x' +
        length.toString(16) +
        '), error="' +
        error.message +
        '"';
